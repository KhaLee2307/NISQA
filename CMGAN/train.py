...
parser.add_argument("--cut_len", type=int, default=48000*2, help="cut length, default is 2 seconds in denoise "
                                                                 "and dereverberation")
...
parser.add_argument("--gen_model_dir", type=str, default='',
                    help="dir of pretrained generator model")
parser.add_argument("--dis_model_dir", type=str, default='',
                    help="dir of pretrained discriminator model")
...
class Trainer:
    def __init__(self, train_ds, test_ds, gpu_id: int, args):
        self.args = args
        self.n_fft = 400
        self.hop = 100
        ...
        if (args.gen_model_dir != ''):
            self.model.load_state_dict((torch.load(args.gen_model_dir)))
        # summary(
        #     self.model, [(1, 2, args.cut_len // self.hop + 1, int(self.n_fft / 2) + 1)]
        # )
        self.discriminator = discriminator.Discriminator(ndf=16).cuda()
        if (args.dis_model_dir != ''):
            self.discriminator.load_state_dict((torch.load(args.dis_model_dir)))
        # summary(
        #     self.discriminator,
        #     [
    ...
    def train(self):
        ...
        for epoch in range(args.epochs):
            ...
            idx = 0
            for batch in tqdm(self.train_ds):
                step = idx + 1
                idx = idx + 1
            ...
            path_dis = os.path.join(
                args.save_model_dir,
                "Discriminator_epoch_" + str(epoch) + "_" + str(gen_loss)[:5],
            )
            if not os.path.exists(args.save_model_dir):
                os.makedirs(args.save_model_dir)
            if self.gpu_id == 0:
                torch.save(self.model.module.state_dict(), path_gen)
                torch.save(self.discriminator.module.state_dict(), path_dis)
            scheduler_G.step()
            scheduler_D.step()
